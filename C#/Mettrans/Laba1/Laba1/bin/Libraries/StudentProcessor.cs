...
namespace Laba1.Libraries
{
    class StudentProcessor
    {
        ...
        // Нахождение максимального числа тезок
        public static (int maxCount, List<string> mostCommonNames) FindMaxTeski(List<string> students)
        {
            ...
            foreach (var student in students)
            {
                ...
                if (parts.Length >= 3)
                {
                    var firstName = parts[1];  // Имя
                    var middleName = parts[2]; // Отчество
        ...
        // Метод для открытия файла и вывода результата (удобный общий метод)
